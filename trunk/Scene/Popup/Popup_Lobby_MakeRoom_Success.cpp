...
Popup_Lobby_MakeRoom_Success::Popup_Lobby_MakeRoom_Success(void)
{
	//< 배경판 이미지
...
//< 초기화
void Popup_Lobby_MakeRoom_Success::init( void )
{
	//< 지정 이미지 로드
	loadImage();
...
//< 업데이트
void Popup_Lobby_MakeRoom_Success::update( POINT mousePos )
{
	//< 버튼 
	...
	{
		//< 상태 돌리기
		//button_OK->inCheckMouseOn( false );
		button_OK->inClickButton( false );
		button_OK->inPlayButtonAni( false );
		//< 팝업 끄기
		button_STATE = POPUP_NO_POPUP_GET_MAKE_ROOM;
	...
	//< 팝업 멘트
	font->LoadText( "방만들기 성공!!", popupImage->getRect(), 
		"HY헤드라인M",
		30, ALIGN_CENTER, 255,255,255 );
}

//< 랜더
void Popup_Lobby_MakeRoom_Success::render( HDC hdc )
{
	//< 투명 이미지 랜더
...
//< 지우기
void Popup_Lobby_MakeRoom_Success::release( void )
{
	//< 불러온 이미지 지우기
...
//< 이미지 불러오기
void Popup_Lobby_MakeRoom_Success::loadImage( void )
{
	//< 알파 이미지
...
//< 이벤트처리
LRESULT	Popup_Lobby_MakeRoom_Success::StateProc( HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam )
{
	switch( msg )
	{
	//< 마우스를 땔때 
