...
bool MonsterMgr::beShotCheck( character &player )
{
	...
	if (false == player.IsPlayingProjectile(E_SkillSlot::NormalAttack))
	{
		return false;
	}
	...
	for( size_t i=0 ; i<m_monsterList.size() ; i++ )
	{
		if( NULL != m_monsterList[i] )
		{
			if (true == collision::isColCirAndRect(m_monsterList[i]->getRect(), player.GetProjectileRect(E_SkillSlot::NormalAttack)))
			{
				//< 몬스터가 살아있다면 검사	
				if( m_monsterList[i]->getStateFlag() != STATE_DIE )
                {
					...
					player.SetProjectileFlag(E_SkillSlot::NormalAttack, false);
					//< 한마리만 피격 받고 종료
					return true;
				}
			}
...
{
	//< 캐릭터 공격 충돌체가 비활성화 상태면 종료
	if (false == player.IsPlayingProjectile(E_SkillSlot::Active_ShootWhole))
	{
		return false;
	}
	for( size_t i=0 ; i<m_monsterList.size() ; i++ )
	{
		if( NULL != m_monsterList[i] )
		{
			if (true == collision::isColCirAndRect(m_monsterList[i]->getRect(), player.GetProjectileRect(E_SkillSlot::Active_ShootWhole)))
			{
				//< 몬스터가 살아있다면 검사
				if( m_monsterList[i]->getStateFlag() != STATE_DIE )
				{
